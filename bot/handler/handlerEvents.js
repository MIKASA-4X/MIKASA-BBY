...
function getRole(threadData, senderID) {
    ...
    const vipUsers = global.db.vipUsers || [];  // Assuming VIP users are stored in the db

    if (!senderID) return 0;

    const adminBox = threadData ? threadData.adminIDs || [] : [];
    
    // Check if sender is an adminBot or thread admin
    if (adminBot.includes(senderID)) return 2;
    if (adminBox.includes(senderID)) return 1;
    
    // Check if sender is a VIP
    if (vipUsers.includes(senderID)) return 4;  // VIP Role

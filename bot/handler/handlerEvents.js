...
function getRole(threadData, senderID) {
	...
	const vipUsers = global.GoatBot.config.vipUsers || []; // ***MODIFICATION 1: SIMULATE VIP USERS LIST***
	if (!senderID)
		return 0;
	const adminBox = threadData ? threadData.adminIDs || [] : [];
	// ***MODIFICATION 2: ADD ROLE 4 FOR VIP USERS***
	return vipUsers.includes(senderID) ? 4 : adminBot.includes(senderID) ? 2 : adminBox.includes(senderID) ? 1 : 0;
}
...
function getText(type, reason, time, targetID, lang) {
	...
	// ***MODIFICATION 3: ADD NEW TEXT TYPE FOR VIP ROLE***
	else if (type == "onlyVIP")
		return "you are not vip user baby.."; // CUSTOM REPLY FOR ROLE 4
}
	...
	return async function (event, message) {
		...
		*/
		...
		async function onStart() {
			...
			if (needRole > role) {
				// ***MODIFICATION 4: CUSTOM VIP USER CHECK AND MESSAGE***
				if (needRole == 4) {
					return await message.reply(getText("onlyVIP", null, null, null, langCode));
				}
				// ***END MODIFICATION 4***
		...
		*/
		async function onReply() {
			...
			if (needRole > role) {
				// ***MODIFICATION 5: CUSTOM VIP USER CHECK AND MESSAGE FOR onReply***
				if (needRole == 4) {
					return await message.reply(getText("onlyVIP", null, null, null, langCode));
				}
				// ***END MODIFICATION 5***
		...
		*/
		async function onReaction() {
			...
			if (needRole > role) {
				// ***MODIFICATION 6: CUSTOM VIP USER CHECK AND MESSAGE FOR onReaction***
				if (needRole == 4) {
					return await message.reply(getText("onlyVIP", null, null, null, langCode));
				}
				// ***END MODIFICATION 6***
